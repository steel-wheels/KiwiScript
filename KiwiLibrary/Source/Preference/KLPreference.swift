...
@objc public class KLTerminalPreference: NSObject, KLTerminalPreferenceProtocol
{
	...
	public var textColor: JSValue {
		get {
			let vpref = CNPreference.shared.viewPreference
			let color = vpref.terminalForegroundColor()
	...
	public var backgroundColor: JSValue {
		get {
			let vpref = CNPreference.shared.viewPreference
			let color = vpref.terminalBackgroundColor()
			return JSValue(int32: color.escapeCode(), in: mContext)
		}
		set(newval) {
			if newval.isNumber {
...
@objc public class KLUserPreference: NSObject, KLUserPreferenceProtocol
{
	...
	public var language: JSValue { get {
		let pref = CNPreference.shared.userPreference
		let lang = pref.language
		return JSValue(int32: Int32(lang.rawValue), in: mContext)
	}}
}
