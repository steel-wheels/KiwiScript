...
open class KECompiler
{
	...
	public func compileBase(context ctxt: KEContext, console cons: CNFileConsole, config conf: KEConfig) -> Bool {
		/* Set strict */
		setStrictMode(context: ctxt, console: cons, config: conf)
		/* Define Enum Types */
		compileTypeDefinitions(context: ctxt, console: cons, config: conf)
		return true
	}
	...
	private func compileTypeDefinitions(context ctxt: KEContext, console cons: CNConsole, config conf: KEConfig) {
		let vmgr    = CNValueTypeManager.shared
		let names   = vmgr.typeNames
		let srcfile = URL(fileURLWithPath: #file)
		for name in names  {
			if let vtype = vmgr.search(byName: name) {
				if let scr = vtype.toScript() {
					let text = scr.toStrings().joined(separator: "\n")
					let _ = compileStatement(context: ctxt, statement: text, sourceFile: srcfile, console: cons, config: conf)
				}
			}
		}
	}
